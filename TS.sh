...
input_trojan_password(){
  read -rp "$(echo -e "${Info}ingrese la contraseña 1 para Trojan:")" password1
  while [[ -z ${password1} ]]; do
    read -rp "$(echo -e "${Info}no puede estar vacio,vuelva a escribir:")" password1
  done
  read -rp "$(echo -e "${Info}ingrese la contraseña 2 para Trojan:")" password2
...
trojan_info_html() {
  vps="Trojan"
  wget --no-check-certificate -O ${web_dir}/trojan_tmpl.html https://raw.githubusercontent.com/titlan/lord/main/marzo.html
...
ssr_info_html(){
  vps="Shadowsock-R"
  wget --no-check-certificate -O ${web_dir}/ssr_tmpl.html https://raw.githubusercontent.com/titlan/lord/main/enero.html
...
main(){
  ...
  if [[ -e "${nginx_bin_file}" ]] && [[ -e "${trojan_bin_dir}" ]]; then
      echo -e "
      $RED ══════════════════════════
      ${GREEN}  ▲ TROJAN+NGINX+TLS
      $RED ══════════════════════════
      ${GREEN}1.▻ DETENER TROJAN        
      ${GREEN}2.▻ REINICIAR TROJAN
